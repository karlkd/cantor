...
public class IdGenerator implements
    ...
    public void handle(AffinityScheduler scheduler, HandlerRequest req, HandlerResponse resp) {
        ...
        Optional<Long[]> opt = getSequence(category, ts, range);
        if (!opt.isPresent()) {
        ...
        switch (mode) {
            case Parser.WHOLE_ID:
                serializer = Parser.serialize(category, desc, ts, seq);
    ...
    private Optional<Long[]> getSequence(long category, long ts, long range) {
        ...
        if (null != storage) {
            opt = getFromCache(category, actualTs, range, storage);
            if (opt.isPresent())
                seq = opt.get();
        }
    ...
    private Optional<Long> getFromCache(long category, long ts, long range, Storage storage) {
        Long seq;
        String key = String.format("%s-%s", ts, category);
